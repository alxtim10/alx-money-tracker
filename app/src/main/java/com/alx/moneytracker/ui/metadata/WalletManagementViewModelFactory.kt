...
/**
 * Manual-DI [ViewModelProvider.Factory] for [WalletManagementViewModel] (Scope 3).
 *
 ...
 */
class WalletManagementViewModelFactory(
    private val repository: MetadataRepository
) : ViewModelProvider.Factory {

    @Suppress("UNCHECKED_CAST")
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        require(modelClass.isAssignableFrom(WalletManagementViewModel::class.java)) {
            "Unknown ViewModel class: ${modelClass.name}"
        }
        return WalletManagementViewModel(repository) as T
    }
}
